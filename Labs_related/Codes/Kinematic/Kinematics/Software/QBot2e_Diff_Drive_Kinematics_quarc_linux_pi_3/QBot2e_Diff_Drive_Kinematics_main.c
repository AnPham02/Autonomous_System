...
typedef RT_MODEL_QBot2e_Diff_Drive_Ki_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * QBot2e_Diff_Drive_Kinematics(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(QBot2e_Diff_Drive_Kinematics_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: QBot2e_Diff_Drive_Kinematics -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = QBot2e_Diff_Drive_Kinematics();
  if (rtmGetErrorStatus(S) != NULL) {
